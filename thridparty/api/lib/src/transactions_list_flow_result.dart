...
library api.src.transactions_list_flow_result;
...
class TransactionsListFlowResult implements TBase {
  static final TStruct _STRUCT_DESC = TStruct("TransactionsListFlowResult");
  static final TField _TRANSACTION_RESULTS_FIELD_DESC = TField("transactionResults", TType.LIST, 1);

  List<TransactionFlowResult> _transactionResults = [];
  static const int TRANSACTIONRESULTS = 1;


  TransactionsListFlowResult() {
  }

  // transactionResults
  List<TransactionFlowResult> get transactionResults => this._transactionResults;

  set transactionResults(List<TransactionFlowResult> transactionResults) {
    this._transactionResults = transactionResults;
  }

  @override
  read(TProtocol iprot) {
    bool __isset_transactionResults = false;
    TField field;
          ...
          if (field.type == TType.LIST) {
            {
              ...
              for (int _i175 = 0; _i175 < _list174.length; ++_i175) {
                TransactionFlowResult _elem176;//declare element for this.transactionResults
                _elem176 = TransactionFlowResult();
                _elem176.read(iprot);
                this.transactionResults.add(_elem176);//add element to parent this.transactionResults
              }
              iprot.readListEnd();
            }
          } else {
  ...
  write(TProtocol oprot) {
    ...
    oprot.writeFieldBegin(_TRANSACTION_RESULTS_FIELD_DESC);
    {
      oprot.writeListBegin(TList(TType.STRUCT, this.transactionResults.length));
      for (var elem177 in this.transactionResults) {
        elem177.write(oprot);
      }
      oprot.writeListEnd();
    }
  ...
  @override
  String toString() {
    StringBuffer ret = StringBuffer("TransactionsListFlowResult(");

    ret.write("transactionResults:");
    ret.write(this.transactionResults);
