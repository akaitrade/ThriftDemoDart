...
library api.src.filtered_transactions_list_result;
...
class FilteredTransactionsListResult implements TBase {
  static final TStruct _STRUCT_DESC = TStruct("FilteredTransactionsListResult");
  static final TField _STATUS_FIELD_DESC = TField("status", TType.STRUCT, 1);
  static final TField _QUERY_RESPONSE_FIELD_DESC = TField("queryResponse", TType.LIST, 2);

  t_general.APIResponse _status = t_general.APIResponse();
  static const int STATUS = 1;
  List<PublicKeyTransactions> _queryResponse = [];
  static const int QUERYRESPONSE = 2;


  FilteredTransactionsListResult() {
  }
  ...
  // queryResponse
  List<PublicKeyTransactions> get queryResponse => this._queryResponse;

  set queryResponse(List<PublicKeyTransactions> queryResponse) {
  ...
  read(TProtocol iprot) {
    ...
    bool __isset_queryResponse = false;
    TField field;
          ...
          if (field.type == TType.LIST) {
            {
              ...
              for (int _i75 = 0; _i75 < _list74.length; ++_i75) {
                PublicKeyTransactions _elem76;//declare element for this.queryResponse
                _elem76 = PublicKeyTransactions();
                _elem76.read(iprot);
                this.queryResponse.add(_elem76);//add element to parent this.queryResponse
              }
              iprot.readListEnd();
            }
          } else {
  ...
  write(TProtocol oprot) {
    ...
    oprot.writeFieldEnd();
    oprot.writeFieldBegin(_QUERY_RESPONSE_FIELD_DESC);
    {
      oprot.writeListBegin(TList(TType.STRUCT, this.queryResponse.length));
      for (var elem77 in this.queryResponse) {
        elem77.write(oprot);
      }
      oprot.writeListEnd();
    }
  ...
  @override
  String toString() {
    StringBuffer ret = StringBuffer("FilteredTransactionsListResult(");

    ret.write("status:");
    ret.write(this.status);

    ret.write(", ");
    ret.write("queryResponse:");
    ret.write(this.queryResponse);
