...
library api.src.smart_contract_data_result;
...
class SmartContractDataResult implements TBase {
  static final TStruct _STRUCT_DESC = TStruct("SmartContractDataResult");
  static final TField _STATUS_FIELD_DESC = TField("status", TType.STRUCT, 1);
  static final TField _METHODS_FIELD_DESC = TField("methods", TType.LIST, 2);
  static final TField _VARIABLES_FIELD_DESC = TField("variables", TType.MAP, 3);

  t_general.APIResponse _status = t_general.APIResponse();
  static const int STATUS = 1;
  List<SmartContractMethod> _methods = [];
  static const int METHODS = 2;
  Map<String, t_general.Variant> _variables = Map<String, t_general.Variant>();
  ...
  // methods
  List<SmartContractMethod> get methods => this._methods;

  set methods(List<SmartContractMethod> methods) {
    this._methods = methods;
  }

  // variables
  Map<String, t_general.Variant> get variables => this._variables;

  set variables(Map<String, t_general.Variant> variables) {
          ...
          if (field.type == TType.LIST) {
            {
              ...
              for (int _i117 = 0; _i117 < _list116.length; ++_i117) {
                SmartContractMethod _elem118;//declare element for this.methods
                _elem118 = SmartContractMethod();
                _elem118.read(iprot);
                this.methods.add(_elem118);//add element to parent this.methods
              }
              iprot.readListEnd();
            }
          } else {
          ...
          if (field.type == TType.MAP) {
            {
              ...
              this.variables = Map<String, t_general.Variant>();
              for (int _i120 = 0; _i120 < _map119.length; ++_i120) {
                String _key121;
                t_general.Variant _val122;
                _key121 = iprot.readString();
                _val122 = t_general.Variant();
                _val122.read(iprot);
                this.variables[_key121] = _val122;
              }
              iprot.readMapEnd();
            }
          } else {
  ...
  write(TProtocol oprot) {
    ...
    oprot.writeFieldBegin(_METHODS_FIELD_DESC);
    {
      ...
      for (var elem123 in this.methods) {
        elem123.write(oprot);
      }
      oprot.writeListEnd();
    }
    oprot.writeFieldEnd();
    oprot.writeFieldBegin(_VARIABLES_FIELD_DESC);
    {
      oprot.writeMapBegin(TMap(TType.STRING, TType.STRUCT, this.variables.length));
      for (var elem125 in this.variables.keys) {
        oprot.writeString(elem125);
        this.variables[elem125]!.write(oprot);
      }
      oprot.writeMapEnd();
    }
    oprot.writeFieldEnd();
  ...
  @override
  String toString() {
    StringBuffer ret = StringBuffer("SmartContractDataResult(");
